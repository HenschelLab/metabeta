...
<!-- <tr><td>ENVO <input type="radio"  name="ch" value="compman.gif" onClick="CB(this.value);" /> </td> -->
</table>
<br><br>""" 
...
function changeImage(url) {
    document.getElementById("PC12").src = url + "/" + "PC1_vs_PC2_plot.png";
    document.getElementById("PC32").src = url + "/" + "PC3_vs_PC2_plot.png";
    document.getElementById("PC13").src = url + "/" + "PC1_vs_PC3_plot.png";
}
</script>
...
IMG_SRC1 = """<img id="%s" src="%s" border=0 />"""
DOWNLOAD_LINK = """<a href="%s" >%s</a>"""
RADIO = """<input type="radio"  name="ch" value="%s" onClick="changeImage(this.value);" /> %s"""
AREA_SRC = """<AREA shape="circle" coords="%d,%d,3" href="#%s"  \
onmouseover="return overlib('%s');" onmouseout="return nd();">\n"""
IMG_MAP_SRC = """<img src="%s" border="0" ismap usemap="#points%s" width="%d" height="%d" />\n"""
IMG_MAP_SRC = """<img id="%s" src="%s" border="0" ismap usemap="#points%s" width="%d" height="%d" />\n"""
...
def make_interactive_scatter(plot_label,dir_path,data_file_link, 
    ...
    savefig(os.path.join(dir_path,img_name), dpi=80,facecolor=background_color)
    svg_img_name = str(x_label[0:3]+'vs'+y_label[0:3]+'plot.svg')
    savefig(os.path.join(dir_path,svg_img_name),format='svg')
    #Create zipped eps files
    eps_link = ""
    if generate_eps:
    ...
    return IMG_MAP_SRC % (props["imgID"], os.path.join(data_file_link,img_name), points_id, 
